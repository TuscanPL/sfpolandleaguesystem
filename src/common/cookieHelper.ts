...
function getMappedUserCookie(userCookieStringified?: string): UserCookie | null {
  const stringifiedCookie =
    userCookieStringified === undefined ? getUserCookie() : userCookieStringified
  ...
  const userCookieData = data.user.identities.filter((identity: any) => {
    if (identity.provider === 'discord') return identity
  })[0]

  if (userCookieData === undefined) return null
